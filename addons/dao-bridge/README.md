...
- **被控端接入（一行·前台临时）**：Windows `iwr http://<公网IP>:9930/api/bootstrap.ps1 | iex`；
  Linux/macOS `curl -fsSL http://<公网IP>:9930/api/bootstrap.sh | sh`。出站长轮询登记为一个 agent（关窗即断，适合一次性诊断）。
- **被控端接入（一行·持久化后端·推荐）**：Windows `iwr http://<公网IP>:9930/api/install.ps1 | iex`；
  Linux/macOS `curl -fsSL http://<公网IP>:9930/api/install.sh | sh`。落盘 `~/.dao/hub-agent.*` 并登记
  Windows 计划任务 `DaoHubAgent`（登录/开机自启·崩溃 1 分钟自愈）或 systemd `--user` 服务（无则 cron `@reboot`），
  **立即后台启动**——机器重启/断线自动重连，无需每次手动跑命令。卸载：
  `Unregister-ScheduledTask -TaskName DaoHubAgent -Confirm:$false`（Win）/ `systemctl --user disable --now dao-hub-agent`（Linux）。
