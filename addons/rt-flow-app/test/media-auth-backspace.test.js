...
ok(/__rtBsGuard6/.test(main), "退格根治 v6: 幂等守卫存在");
ok(!/__rtBsGuard5/.test(main) && !/__rtBsGuard4\)/.test(main) && !/__rtBsGuard3/.test(main) && !/__rtBsGuard2/.test(main), "退格根治 v6: v2~v5 旧守卫已整体移除");
...
ok(/addEventListener\('beforeinput'/.test(bsFlat), "退格根治 v6: 捕获层监听 beforeinput");
ok(/deleteContentBackward/.test(bsFlat) && /deleteContentForward/.test(bsFlat), "退格根治 v6: 只拦删除类 inputType (其余直通)");
ok(/data-slate-editor/.test(bsFlat), "退格根治 v6: 只对 slate 编辑器生效 (普通输入框不受影响)");
ok(/e\.cancelable&&editor/.test(bsFlat), "退格根治 v6: cancelable+editor 才走纯模型路线 (否则回退 v5 兜底)");
ok(/preventDefault\(\)/.test(bsFlat), "退格根治 v6: preventDefault 掐掉 Chromium DOM 直改 (单一记账·消跳动卡顿之根)");
ok(/deleteBackward\('character'\)/.test(bsFlat) && /deleteForward\('character'\)/.test(bsFlat), "退格根治 v6: 塌缩点删除交 Slate 模型 deleteBackward/deleteForward");
ok(/deleteFragment\(\)/.test(bsFlat), "退格根治 v6: 选区删除走 deleteFragment (选中整段删除正确)");
ok(/stopImmediatePropagation/.test(bsFlat), "退格根治 v6: sIP 拦下 Slate 二次记账");
ok(/,true\);/.test(bsFlat), "退格根治 v6: 捕获阶段安装 (先于 Slate 处理器)");
ok(/getTargetRanges/.test(bsFlat), "退格根治 v6: getTargetRanges 起止映射模型点");
ok(/__reactFiber/.test(bsFlat), "退格根治 v6: fiber 取 Slate editor 实例");
ok(/set_selection/.test(bsFlat), "退格根治 v6: editor.apply(set_selection) 先归正模型选区");
ok(/addEventListener\('input'/.test(bsFlat), "退格根治 v6: 保留 v5 兜底 (不可取消时落地后归正)");
ok(!/selectionchange/.test(bsGuard), "退格根治 v6: 无 selectionchange 光标干预");
ok(!/execCommand|dispatchEvent/.test(bsFlat), "退格根治 v6: 零 execCommand/零合成事件 (不掐 IME 会话)");
...
ok(/installBackspaceGuard\(v\); installVideoFit\(v\); installMediaRetry\(v\); installAttachmentPrefetch\(v\); harvestPageAuth\(v, tab, u\); warmAttachmentCookie/.test(main), "SPA 路由后重采 (doUpdateVisitedHistory)");
