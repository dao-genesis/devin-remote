...
ok(/__rtBsGuard5/.test(main), "退格根治 v5: 幂等守卫存在");
ok(!/__rtBsGuard4\)/.test(main) && !/__rtBsGuard3/.test(main) && !/__rtBsGuard2/.test(main), "退格根治 v5: v2/v3/v4 旧守卫已整体移除");
...
ok(/addEventListener\('beforeinput'/.test(bsFlat), "退格根治 v5: 捕获层监听 beforeinput");
ok(/deleteContentBackward/.test(bsFlat) && /deleteContentForward/.test(bsFlat), "退格根治 v5: 只拦删除类 inputType (其余直通)");
ok(/data-slate-editor/.test(bsFlat), "退格根治 v5: 只对 slate 编辑器生效 (普通输入框不受影响)");
ok(/stopImmediatePropagation/.test(bsFlat), "退格根治 v5: sIP 拦下 Slate 二次记账 (DOM 删除由 Chromium 落地·MutationObserver 单次对账)");
ok(/,true\);/.test(bsFlat), "退格根治 v5: 捕获阶段安装 (先于 Slate 处理器)");
ok(/getTargetRanges/.test(bsFlat), "退格根治 v5: beforeinput 时快照删除区起点绝对偏移");
ok(/__reactFiber/.test(bsFlat), "退格根治 v5: fiber 取 Slate editor 实例");
ok(/set_selection/.test(bsFlat), "退格根治 v5: editor.apply(set_selection) 归正模型光标 (光标跳末尾/长按整段误删/键盘割裂之解)");
ok(/addEventListener\('input'/.test(bsFlat), "退格根治 v5: input 落地后再归正 (不掐 MutationObserver 对账)");
ok(!/selectionchange/.test(bsGuard), "退格根治 v5: 无 selectionchange 光标干预");
ok(!/execCommand|dispatchEvent/.test(bsFlat), "退格根治 v5: 零 execCommand/零合成事件 (不掐 IME 会话)");
