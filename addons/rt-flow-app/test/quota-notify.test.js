...
ok(/if\(QUOTA_RE\.test\(qsig\) && qLive!==true\) return "quota";\s*\n\s*if\(actionReq\) return "action_required";/.test(engineSrc),
   "源级: convReasonOf 额度信号最高优先(且经账号实时额度对账·满额号不计耗尽)");
...
ok(/var rs=String\(\(o&&o\.reason\)\|\|""\)\.toLowerCase\(\);/.test(engineSrc) &&
   /qsig=rs\+" "\+String\(s\.status\|\|""\)\+" "\+en\+" "\+String\(s\.activity_status\|\|""\)\+" "\+String\(s\.current_activity/.test(engineSrc),
