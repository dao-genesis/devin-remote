...
function _dvOverviewHtml(ov, i) {
  ...
  const credits =
    ov.billing && (ov.billing.overage_credits != null || ov.billing.credits != null)
      ? "额度 " + (ov.billing.overage_credits != null ? ov.billing.overage_credits : ov.billing.credits)
  ...
  for (const s of sess) {
    ...
    const stTxt = cls === "running" ? "运行" : cls === "blocked" ? "卡住" : cls === "finished" ? "完成" : "空闲";
    h +=
...
async function _dvRunPoll() {
  try {
    ...
    for (const acc of _store.accounts) {
      ...
      if (!auth) continue;
      try {
        const running = await devinCloud.listRunningSessions(auth);
        if (running.length) {
          items.push({ email: key, running: running.length, titles: running.map((r) => r.title) });
          for (const r of running) _dvMaybeNotify(acc.email, r);
        }
        _dvDetectFinished(acc.email, running);
      } catch {}
    }
    _broadcastMsg({ type: "devinRunStatus", items });
  } catch {}
}
// 运行/卡住/完成 通知 (左下角 IDE) · 去抖
...
function _dvMaybeNotify(email, sess) {
  ...
  if (sess.statusClass === "blocked" && prev !== "blocked") {
    _notify("warn", "[" + (devinCloud.getTag(email) || email.split("@")[0]) + "] 对话疑似卡住/额度超限: " + sess.title);
  }
...
async function activate(context) {
  ...
  // ── 第五板块 · Devin Cloud 初始化 (运行状态轮询 + 可选自动备份) ──
  try {
    if (_cfg("devinCloudAutoBackup", false)) _dvStartAuto();
    ...
    log("devin-cloud: 第五板块就绪 · autoBackup=" + _cfg("devinCloudAutoBackup", false) + " pollMin=" + pollMin);
  } catch (e) {
