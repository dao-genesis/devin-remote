...
- **近期对话·非本人自动化对话隔离(下沉/可隐藏·借鉴手机 APK cloud.html·帛书「知其白·守其黑」)。** 桌面「🕒 近期对话」此前把每周烧额度的自动化会话与本人对话混排、扰乱时序:
  ...
  - 近期列表本人对话恒在前、自动化**下沉沉底**(不删数据·半透明标注), 新增 **🙈 隐藏自动化 (N)** 一键切换(下沉↔隐藏);
  - 时序不再被自动化会话时间顶起 → 修正「近期对话显示错误」。

## 3.51.21
- **对话备份·出库账号本地可查/加回(借鉴手机 APK·帛书「复守其母·没身不殆」)。** 备份树 `listBackups` 早已全盘扫描(含已移出账号池的出库账号), 但面板未把「当前账号池」与「历史备份」解耦呈现, 出库账号看不出、加不回:
  - `daoAnnotateBackupPool` 给每个备份账号标注 `inPool`(是否仍在当前账号池)——出库账号(移出但本地备份仍在)在「按账号」视图打 **已出库** 徽章, 仍可搜索/展开/解锁/查看正文;
  - `daoBackupFolderPwd` 从备份目录名 `<编号?>_<邮箱本地名>_<密码?>` 尽力恢复密码(best-effort·safeName 处理过, 可能失真);
  - 新增 **↩ 加回账号库**(`reAddBackupAccount`): 把出库账号按【普通新账号】追加进切号真池 `~/.wam/accounts.md`(去重·幂等), 不因历史移除而阻止; 无法恢复密码时引导「添加账号」手动加回。凭据仅落地本地真池, 不回显/不入日志。
  - 当前账号池数量决定当前账号列表, 历史备份独立保留 —— 二者解耦。

## 3.51.10
- **新增「🏛 组织管理 body/glove 操作台」(反向注入板块·通用能力)。** 任何用户只需提供自己主账号的一枚 PAT(scopes: admin:org+repo), 插件后端跑通全流程:
  - `daoOrgOverview` 验 PAT 有效性+权限 scopes+列出已有组织(无组织时给 GitHub 免费新建指引);
  - `daoOrgSyncRepos` 把 owner/repo 批量转移进组织(数据落本体·限速防风控);
  - `daoOrgInvite` 批量把手套账号(各 Devin 自带 GitHub 身份)拉入组织为成员(限速+尊重 rate-limit reset);
  - 本体组织+手套清单持久化进注入档案(`orgBody`), 切号自动带上下文。不硬编任何特定组织, 每个用户用自己的 org/PAT。
- **修复「账号归零/出库后本地对话备份随账号一起消失」(对照手机 APK 同源病灶)。** 实盘取证: 备份本体从未被删(台式机 E:\DaoDevinBackups 611 账号 / home 旧根 276 账号 均在盘), “消失”全在列表侧 `listBackups` 三处盲区:
  - **无 `_meta.json` 的旧格式对话文件夹被整个跳过**(有 `对话.md`/`对话.html` 正文也计 0 条) —— 现凭正文即计入, 标题据文件夹名合成、短 ID 反查 devinId。
  - **备份根切盘(home → 数据盘)后旧根不再被扫** —— 现 `listBackups` 聚合主根 + home 旧根 + 历史持久化根, 同名账号跨根合并(对话按名去重), 旧根备份永不隐身。
  - **裸邮箱命名的账号目录无 `.account.json` 时 email 空** → 按号检索(_dvLocalOverview)失配 —— 现目录名含 `@` 即回填 email。
  出库链路(`removeBatch`/`_purgeAccountResiduals`)本就只清状态不碰备份目录, 维持不变: 备份 = 不可变档案, 账号去留与它无关。

## 3.50.116
- **翻译跨源自愈补强(对照手机 APK 闭环)**：
  - 重载标记从「每标签一次性布尔」改为**按 URL 记忆**(`__trRerouted===url`)——此前一旦某页自愈失败，同标签后续任何页面点翻译都直接报「本页不可翻译(跨源)」永不再试；现换页即重新获得一次 /__web 自愈机会。
  - 经 /__web 重载后从「load 后定时 500ms 单发续译」改为**轮询等 doc 就绪(500ms×20·至多 10s)**——SPA 水合/多级重定向晚于 load 事件时不再空枪，就绪即续译。
...
- **切号板块三大修复(根治「面板自动跳顶 + 新账号初始化没配好 + 归零清理不生效」)**。
  ...
  - **新账号系统级初始化**: 原 `!p.enabled → skip` 把固定道藏(准则KB/桥KB/DAO_TOKEN)也一起跳过 → 新添加账号根本不初始化。改为: 系统级注入不受用户档案 enabled 门控, 用户自选项仍受控。
  - **立即清理按钮(参手机版)**: 在切号面板增加「⚡ 立即清理(全部)」按钮, 委托 `wam.devinCleanupImmediate`(无模态·全程 toast), 与现有「🌊 归零清理」并列。
