...
// ───────────────────────────────────────────────────────────────────────────
// 道: 数据本体(org·低频·几乎不被封) + 手套(各 Devin 自带 GitHub 号·高频自动化·可弃换)。
    ...
    return new Promise((resolve) => {
        ...
        const req = https.request({ hostname: 'api.github.com', path: apiPath, method, headers }, (res: any) => {
            ...
            res.on('end', () => {
                ...
                const rt = Number(res.headers['x-ratelimit-reset']);
                const out: any = {
...
const _ghSleep = (ms: number) => new Promise<void>((r) => setTimeout(r, ms));

// PAT 概览: 验证 + viewer 身份 + 权限(scopes) + 可管理组织清单(role=admin 才能拉人)。
async function daoOrgOverview(pat: string): Promise<{ ok: boolean; login?: string; name?: string; scopes?: string; canAdminOrg?: boolean; orgs?: { login: string; role: string; state: string }[]; error?: string }> {
    pat = String(pat || '').trim();
    if (!pat) return { ok: false, error: '未提供 PAT' };
    const u = await ghApiRequest('GET', '/user', pat);
    if (u.status !== 200 || !u.json) return { ok: false, error: 'PAT 无效或不可用 (HTTP ' + u.status + (u.json && u.json.message ? ' · ' + u.json.message : '') + ')' };
    const scopes = String(u.scopes || '');
    const orgs: { login: string; role: string; state: string }[] = [];
    for (let page = 1; page <= 5; page++) {
        const r = await ghApiRequest('GET', '/user/memberships/orgs?per_page=100&page=' + page, pat);
        if (r.status !== 200 || !Array.isArray(r.json) || !r.json.length) break;
        for (const m of r.json) orgs.push({ login: (m.organization && m.organization.login) || '', role: m.role || '', state: m.state || '' });
        if (r.json.length < 100) break;
    }
    const canAdminOrg = scopes.split(',').map(s => s.trim()).some(s => s === 'admin:org' || s === 'write:org');
...
async function daoOrgInviteMembers(pat: string, org: string, logins: string[], role: string, onProgress?: (done: number, total: number, last: any) => void): Promise<{ ok: boolean; org: string; results: { login: string; ok: boolean; state?: string; error?: string }[] }> {
    ...
    const results: { login: string; ok: boolean; state?: string; error?: string }[] = [];
    const uniq = Array.from(new Set(logins.map(s => String(s || '').trim().replace(/^@/, '')).filter(Boolean)));
    for (let i = 0; i < uniq.length; i++) {
        const login = uniq[i];
        const r = await ghApiRequest('PUT', '/orgs/' + encodeURIComponent(org) + '/memberships/' + encodeURIComponent(login), pat, { role });
        const item = (r.status === 200 && r.json)
            ? { login, ok: true, state: r.json.state }
            : { login, ok: false, error: (r.json && r.json.message) || r.error || ('HTTP ' + r.status) };
        results.push(item);
        if (onProgress) { try { onProgress(i + 1, uniq.length, item); } catch { /* 守柔 */ } }
        // 限速退避: 余量将尽按 reset 等待; 常态每次间隔避免高频风控。
        if (r.rateRemaining !== undefined && r.rateRemaining <= 2 && r.rateReset) {
            const wait = Math.max(0, r.rateReset * 1000 - Date.now()) + 1000;
            if (wait > 0 && wait < 3600000) await _ghSleep(wait);
...
// 同步仓库进组织: 对每个 owner/repo POST /repos/{owner}/{repo}/transfer (new_owner=org)。
async function daoOrgSyncRepos(pat: string, org: string, repos: string[]): Promise<{ ok: boolean; org: string; results: { repo: string; ok: boolean; error?: string }[] }> {
    pat = String(pat || '').trim(); org = String(org || '').trim();
    ...
    for (let i = 0; i < uniq.length; i++) {
        const full = uniq[i]; const parts = full.split('/');
        if (parts.length !== 2 || !parts[0] || !parts[1]) { results.push({ repo: full, ok: false, error: '格式应为 owner/repo' }); continue; }
        const r = await ghApiRequest('POST', '/repos/' + encodeURIComponent(parts[0]) + '/' + encodeURIComponent(parts[1]) + '/transfer', pat, { new_owner: org });
        ...
        if (i < uniq.length - 1) await _ghSleep(1500);
    }
    return { ok: results.some(x => x.ok), org, results };
}
...
function bridgePersistentChannelSectionMd(tok: string): string {
    ...
    return [
        ...
        '',
        '> 恒定地址·永不漂。与快速通道同 Token 同 API, 互为备份: 快速通道不可达时直接改用本地址。',
        '',
        '```',
        `中继URL:   ${relayUrl}`,
        `Token:     ${tok}`,
...
function bridgeGenerateCloudMd(pair?: { url: string; token: string }): string {
    ...
    return [
        ...
        '## ⚡ 断线零人工自愈(务必先读)',
        '',
        '> 公网URL 随隧道重启而变; 插件自动刷新隧道并把新 URL 反向注入回本条目。**永不找用户重发地址——遇死即重读本条目**(看「更新于」时间戳):',
        '',
        '1. `GET /api/health` **2xx=活** → 直接操作。',
        '2. 5xx/超时/DNS 失败 = 隧道已轮换 → 先试持久化通道; 仍不通则重读本条目(≤30s 内自动改写为新 URL)。',
        '3. 重读仍旧址 = 正在重建 → 每 30s 重读, 最多 5 分钟; 新 URL `/api/health` 通即接管。',
        '4. > 5 分钟仍无可达 URL 才提示用户重启 IDE(极少发生)。',
        '',
        '## 🛰️ 去中心化路线C(零中心兜底·ntfy mesh)',
        '',
        '> 终极兜底: 快速通道与 Worker 同时全挂时, 经多家公共 ntfy pub/sub 加密直达本机(任一 broker 可达即通·broker 只见密文·AES-256-GCM)。RPC 帧与整机端点同构, `/api/exec` 等照常可用。',
        '',
        '```',
        ...
        '```',
        '',
        '**Python 客户端(仅需 `cryptography`·实测 RTT≈1.4s):**',
        '```python',
        ...
        '## 无缝接力 · 重启/切号不断连',
        '',
        '> IDE 重启/切号时账号 API 端口会变, 但 Token 机器级恒定、常驻桥不随窗口死。真相文件 `~/.dao/dao-conn-current.json`',
        '> = `{url,token,port,host,pid,epoch,alive:[...]}`; `epoch` 增大即轮换, `alive` 列全部活实例(current=端口最小者)。',
        '',
        '接力: `GET /api/next` 一次拿 `{current,alive,epoch}`; 或经本隧道 `POST /api/exec {"cmd":"type %USERPROFILE%\\\\.dao\\\\dao-conn-current.json"}`。',
        '',
        ...
        '```',
        '',
        '接入后该机出现在 `GET /api/agents` 在线设备中, 可被远程操控(长轮询 `/api/poll` 领命·回传 `/api/result`)。',
        '',
        '## Python SDK',
        '',
        '```python',
...
function bridgeGenerateMcpUsageMd(): string {
    ...
    return [
        ...
        '## 断线自愈',
        '',
        '端点随隧道重启而变, 自愈法与上文「断线零人工自愈」同源: `tools/list` 2xx=活; 失败即重读本条目拿新 `/mcp`(每 30s·最多 5 分钟)。设备/IDE/工具集/版本任一变化, 插件即重生成本文并反向注入。',
        '',
        '*道法自然 · 无为而无不为*',
    ].join('\n');
}
...
async function bridgeLivenessTick(): Promise<void> {
    ...
    try {
        ...
        // 独立闭环(鸡犬相闻·不相往来): 不读外部 conn, 自己重启自己的隧道
        try { daoSyncDaoMcpIntoProfile(); } catch { /* 守柔 */ }
        if (!bridgeOk) {
            _bridgeLivenessFail++;
...
var _wamFrameUrl='';
var _wamRebuildTs=0;
// 反者道之动: wamInit 回包可能因宿主回推链路未就绪而丢失(postMiddle 双发但目标为空时静默吞掉)
//   → 客户端自愈: 未就绪则按 4s 节拍重发, 最多 8 次, 就绪即止。
var _wamKickN=0,_wamKickT=null;
...
function rWamMount(html,warn){
  ...
  S._wamReady=true;
  _wamKickN=0;if(_wamKickT){clearTimeout(_wamKickT);_wamKickT=null;}
}
...
// B · 7号面板 GitHub PAT 一键反向注入(覆盖式·与切号侧同源): 输入新 PAT → 覆盖 profile.secrets 的
//   GITHUB_PAT → 保存 → 询问是否立即注入到所有账号(先删旧再注·防 409 保留旧值)。
function ipCurrentPat(){var s=(S.injectProfile&&S.injectProfile.secrets)||[];for(var i=0;i<s.length;i++){if(s[i].name==='GITHUB_PAT')return s[i].value||'';}return '';}
function ipSetPat(){var cur=ipCurrentPat();var mask=cur?(cur.slice(0,7)+'…'+cur.slice(-4)):'(未设置)';
  sm('💉 GitHub PAT 反向注入 (覆盖式)','<p style="font-size:11px;color:var(--muted);line-height:1.6;margin:2px 0 6px">当前: <b>'+esc(mask)+'</b><br>粘贴最新 PAT(经典 ghp_ 或细粒度 github_pat_)→ 覆盖注入清单的 GITHUB_PAT。后续每个新账号切入时自动带此 PAT。</p><input id="m1" placeholder="ghp_… 或 github_pat_…" style="width:100%;margin:4px 0" autocomplete="off"><label style="display:flex;align-items:center;gap:8px;margin:6px 0;font-size:12px"><input type="checkbox" id="m2" checked> 立即覆盖注入到所有账号(先删旧同名密钥再注)</label>',
  function(){var pat=document.getElementById('m1').value.trim();if(!pat)return false;
    if(!/^(ghp_|github_pat_|gho_|ghu_)/.test(pat)){toast('PAT 格式无效(应以 ghp_ / github_pat_ 开头)',false);return false;}
    S.injectProfile.secrets=S.injectProfile.secrets||[];var ex=S.injectProfile.secrets.find(function(s){return s.name==='GITHUB_PAT';});
    if(ex)ex.value=pat;else S.injectProfile.secrets.push({name:'GITHUB_PAT',value:pat});
    ipSave();var pushAll=document.getElementById('m2').checked;rInject();
    if(pushAll)cmd('applyInjectProfileToAll',{pat:pat});else toast('✓ GITHUB_PAT 已更新(下次切号自动注入)',true);})}
function ipRemove(kind,idx){S.injectProfile[kind].splice(idx,1);ipSave();rInject()}
// ── 🏛 组织管理 body/glove 操作台 — 通用: 用户只提供一枚自己的 PAT (admin:org+repo), 插件后端跑通
//    验PAT→列组织(无组织给指引新建)→同步仓库进组织→批量拉手套(各 Devin 自带 GitHub 号)入组为成员。
function orgMgrHtml(p){var ob=p.orgBody||{};var saved=ob.org?('<span style="color:var(--success)">● 本体: '+esc(ob.org)+' · 手套 '+((ob.gloves||[]).length)+' 只</span>'):'<span style="color:var(--muted)">○ 未配置</span>';
  var h='<details style="margin:4px 0 10px" '+(window._orgOpen?'open':'')+' ontoggle="window._orgOpen=this.open"><summary style="cursor:pointer;font-size:11px;color:var(--fg)">🏛 组织管理 body/glove 操作台 — '+saved+'</summary>';
  h+='<div class="card" style="margin-top:6px">';
  h+='<p style="font-size:10px;color:var(--muted);line-height:1.6;margin:2px 0 6px">只需一枚你主账号的 PAT (scopes: <b>admin:org + repo</b>; 建议加 workflow)。留空则用上方注入清单的 GITHUB_PAT。没有组织? 点「检测」后按指引到 GitHub 免费新建一个, 再回来填组织名。</p>';
  h+='<input id="orgPat" type="password" placeholder="PAT (留空=用注入清单的 GITHUB_PAT)" style="width:100%;margin:2px 0;box-sizing:border-box" autocomplete="off">';
  h+='<div style="display:flex;gap:4px;margin:2px 0"><input id="orgName" placeholder="组织名 (本体 org)" value="'+esc(ob.org||'')+'" style="flex:1"><select id="orgRole" style="width:90px"><option value="member"'+(ob.role!=='admin'?' selected':'')+'>member</option><option value="admin"'+(ob.role==='admin'?' selected':'')+'>admin</option></select></div>';
  h+='<textarea id="orgGloves" placeholder="手套 GitHub 用户名 (空格/逗号/换行分隔) — 各 Devin 账号自带的 GitHub 身份" style="width:100%;height:52px;margin:2px 0;box-sizing:border-box;font-family:monospace;font-size:11px">'+esc((ob.gloves||[]).join(' '))+'</textarea>';
  h+='<textarea id="orgRepos" placeholder="要同步进组织的仓库 owner/repo (可多个, 空格/换行分隔; 转移仓库到组织)" style="width:100%;height:40px;margin:2px 0;box-sizing:border-box;font-family:monospace;font-size:11px"></textarea>';
  h+='<div class="br" style="margin:4px 0"><button class="btn sm primary" onclick="orgDetect()" title="验 PAT 有效性 + 权限 scopes + 列出你已有的组织">🔍 检测 PAT / 组织</button><button class="btn sm" onclick="orgSaveBody()" title="把 本体组织+手套清单 记入注入档案(本地), 切号自动带上下文">💾 记住本体</button><button class="btn sm" onclick="orgInvite()" title="批量把手套账号拉入组织为成员(限速防风控)">👥 批量拉手套入组</button><button class="btn sm" onclick="orgSync()" title="把 owner/repo 逐个转移进组织(数据落本体)">📦 同步仓库进组织</button></div>';
  h+='<div id="orgOut" style="font-size:11px;line-height:1.6;margin-top:4px"></div>';
  h+='</div></details>';return h}
function orgArgs(){return {pat:(document.getElementById('orgPat')||{}).value||'',org:((document.getElementById('orgName')||{}).value||'').trim(),role:(document.getElementById('orgRole')||{}).value||'member',gloves:(document.getElementById('orgGloves')||{}).value||'',repos:(document.getElementById('orgRepos')||{}).value||''}}
function orgMsg(html){var o=document.getElementById('orgOut');if(o)o.innerHTML=html}
function orgDetect(){orgMsg('⏳ 检测中…');cmd('daoOrgOverview',{pat:orgArgs().pat})}
function orgSaveBody(){var a=orgArgs();if(!a.org){toast('先填组织名',false);return}cmd('daoOrgSaveBody',{org:a.org,role:a.role,gloves:a.gloves})}
function orgInvite(){var a=orgArgs();if(!a.org||!a.gloves.trim()){toast('需组织名+手套用户名',false);return}orgMsg('⏳ 批量拉手套入组…');cmd('daoOrgInvite',a)}
function orgSync(){var a=orgArgs();if(!a.org||!a.repos.trim()){toast('需组织名+owner/repo',false);return}orgMsg('⏳ 同步仓库进组织…');cmd('daoOrgSyncRepos',a)}
function orgPick(o){var i=document.getElementById('orgName');if(i)i.value=o;toast('已选组织: '+o,true)}
function orgOnProgress(d){orgMsg('⏳ 入组进度 '+d.done+'/'+d.total+(d.last?(' · 最近: '+esc((d.last.login||''))+' '+(d.last.ok?'✓':'✗ '+esc(d.last.error||''))):''))}
function orgOnResult(d){
  if(d.kind==='overview'){
    if(!d.ok){orgMsg('<span style="color:var(--danger)">✗ '+esc(d.error||'PAT 无效')+'</span>'+(String(d.error||'').indexOf('无 PAT')>=0?'':'<br>提示: PAT 需 admin:org + repo scopes'));return}
    var h='✓ <b>'+esc(d.login||'')+'</b> · scopes: <code>'+esc(d.scopes||'')+'</code>'+(d.canAdminOrg?'':' <span style="color:var(--warn)">(缺 admin:org, 无法管理组织成员)</span>')+'<br>';
    var orgs=d.orgs||[];
    if(orgs.length){h+='已有组织(点选): '+orgs.map(function(o){return '<button class="btn sm" onclick="orgPick(&#39;'+esc(o.login)+'&#39;)">'+esc(o.login)+(o.role==='admin'?' ★':'')+'</button>'}).join(' ')}
    else{h+='<span style="color:var(--warn)">无组织</span> → <a href="https://github.com/account/organizations/new?plan=free">GitHub 免费新建组织</a> (30 秒: 起名→选 Free→确认), 建好回来填组织名即可。'}
    if(d.orgBody&&d.orgBody.org)h+='<br>已记住本体: <b>'+esc(d.orgBody.org)+'</b> · 手套 '+((d.orgBody.gloves||[]).length)+' 只';
    orgMsg(h)
  }else if(d.kind==='saved'){toast('✓ 本体配置已记住',true);S.injectProfile.orgBody=d.orgBody;rInject()}
  else if(d.kind==='invite'||d.kind==='sync'){
    if(d.error){orgMsg('<span style="color:var(--danger)">✗ '+esc(d.error)+'</span>');return}
    var rs=d.results||[];var okN=rs.filter(function(x){return x.ok}).length;
    var h='<b>'+(d.kind==='invite'?'入组':'同步')+'完成 '+okN+'/'+rs.length+' → '+esc(d.org||'')+'</b><br>';
...
function rInject(){
  ...
  // B · GitHub PAT 反向注入 专属卡片(醒目·一键覆盖更新)
  (function(){var cur='';var s=p.secrets||[];for(var i=0;i<s.length;i++){if(s[i].name==='GITHUB_PAT'){cur=s[i].value||'';break;}}var mask=cur?(esc(cur.slice(0,7))+'…'+esc(cur.slice(-4))):'<span style="color:var(--warn)">未设置</span>';h+='<div class="st">💉 GitHub PAT 反向注入</div><div class="card" style="border-left:3px solid var(--success)"><div class="cr"><span class="l" style="font-size:12px">当前 PAT</span><span class="v" style="font-size:11px;font-family:monospace">'+mask+'</span></div><div style="padding:6px 0"><button class="btn sm primary" style="width:100%" onclick="ipSetPat()" title="粘贴最新 PAT 覆盖式更新 GITHUB_PAT · 可一键注入到所有账号(先删旧再注)">🔑 更新 / 注入 GitHub PAT</button></div></div>';})();
  // 两种 GitHub 资源管理用法说明 — ① 个人 PAT 直注 ② 组织 body/glove 免长期PAT (多用户/团队适用)
  h+='<details style="margin:4px 0 10px"><summary style="cursor:pointer;font-size:11px;color:var(--muted)">📖 两种用法: 个人 PAT 直注 · 组织 body/glove 免PAT (点开)</summary>'
    +'<div class="card" style="font-size:11px;line-height:1.7;margin-top:6px">'
    +'<p style="margin:2px 0"><b>① 个人用户 (输入自己的 PAT)</b>: 点上方「更新 / 注入 GitHub PAT」粘贴你的 ghp_/github_pat_。插件自动: 把它作为 <b>GITHUB_PAT</b> 密钥反向注入你所有账号的 Security, 并同源钉住官方 <b>GitHub MCP</b> (Authorization=Bearer 该 PAT) 一并注入。此后每个新切入账号自动带最新 PAT, 任何 agent 调 GitHub MCP / gh 即用同一 PAT; 换新 PAT 只需在此重填一次, 全池自动收敛。</p>'
    +'<p style="margin:6px 0 2px"><b>② 组织体系 body/glove (免长期 PAT · 团队/规模化推荐)</b>: 用你的主账号+一个 GitHub 组织做「本体 body」= 数据中枢 (低频操作·几乎不会被封); 把各 Devin 账号自带的 GitHub 身份做「手套 glove」= 高频 fork/commit/PR 批量自动化 (可随时弃换)。流程: 手套 fork 本体仓 → 提交 → 开带 <b>dao-auto</b> 标签的 PR → 本体侧限速工作流自动合并。手套被封即换新手套, 本体数据零损, 从根上规避高频自动化封号。建组织+加手套 allowlist(.dao/automation.yml) 步骤见知识库「DAO 组织·本体/手套(body/glove) 免PAT 管理体系」(反向注入默认带入每个账号)。</p>'
    +'</div></details>';
  // 🏛 组织管理 · body/glove 落地操作台 (通用: 任何用户只提供自己的一枚 PAT, 插件跑通全流程)
  h+=orgMgrHtml(p);
    ...
    try {
        switch (msg.command) {
            ...
            case 'wamInit': {
                ...
                if (!html) { reply({ type: 'wamInitHtml', html: '<!DOCTYPE html><html><head><meta charset="utf-8"></head><body style="font-family:sans-serif;color:#ccc;background:#1e1e1e;padding:24px"><p>⚠ RT Flow 切号引擎未就绪，正在自动重试…</p></body></html>', warn: true }); break; }
            ...
            case 'cleanupImmediate': {
                // 立即清理(参手机版·无模态): 全部账号 先备份→归零→出库, 一气呵成
                try { await vscode.commands.executeCommand('wam.devinCleanupImmediate'); } catch (e: any) { vscode.window.showErrorMessage('立即清理: ' + (e?.message || e)); }
                refreshReply({ type: 'actionResult', command: 'cleanupImmediate', ok: true });
                break;
            }
            case 'openBridgeMd': {
            ...
            // ── 组织管理 · body/glove 批量入组 (通用·用户仅提供一枚自己的 PAT) ──
            // PAT 取值优先级: 显式传入 msg.pat > 注入档案里的 GITHUB_PAT。
            case 'daoOrgOverview': {
                ...
                reply({ type: 'daoOrgResult', kind: 'overview', ...r, orgBody: prof.orgBody || null });
                break;
            }
            case 'daoOrgSaveBody': {
                // 记住选定的中枢组织 + 手套清单(仅本地)
                const prof = loadInjectProfile();
                const gloves = String(msg.gloves || '').split(/[\s,;]+/).map((s: string) => s.trim().replace(/^@/, '')).filter(Boolean);
                ...
                reply({ type: 'daoOrgResult', kind: 'saved', ok: true, orgBody: prof.orgBody });
                break;
            }
            case 'daoOrgInvite': {
                const prof = loadInjectProfile();
                ...
                const logins = String(msg.gloves || '').split(/[\s,;]+/).map((s: string) => s.trim().replace(/^@/, '')).filter(Boolean);
                if (!pat || !org || !logins.length) { reply({ type: 'daoOrgResult', kind: 'invite', ok: false, error: '需 PAT + 组织名 + 至少一个手套 GitHub 用户名' }); break; }
                await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: '批量拉手套入组 ' + org + '…' }, async () => {
                    const r = await daoOrgInviteMembers(pat, org, logins, role, (done, total, last) => {
                        try { reply({ type: 'daoOrgProgress', kind: 'invite', done, total, last }); } catch { /* 守柔 */ }
                    });
                    const okN = r.results.filter(x => x.ok).length;
                    vscode.window.showInformationMessage('批量入组完成: ' + okN + '/' + r.results.length + ' → ' + org);
                    reply({ type: 'daoOrgResult', kind: 'invite', ...r });
                });
                break;
            }
            case 'daoOrgSyncRepos': {
                const prof = loadInjectProfile();
                const pat = String(msg.pat || '').trim() || String((prof.secrets.find(s => s.name === 'GITHUB_PAT') || { value: '' }).value || '').trim();
                const org = String(msg.org || '').trim();
                const repos = String(msg.repos || '').split(/[\s,;]+/).map((s: string) => s.trim()).filter(Boolean);
                if (!pat || !org || !repos.length) { reply({ type: 'daoOrgResult', kind: 'sync', ok: false, error: '需 PAT + 组织名 + 至少一个 owner/repo' }); break; }
                await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: '同步仓库进组织 ' + org + '…' }, async () => {
...
function loadInjectProfile(): InjectProfile {
    try {
        ...
        return {
            ...
            orgBody: (j.orgBody && typeof j.orgBody === 'object') ? {
                org: String(j.orgBody.org || ''),
                role: j.orgBody.role || 'member',
                gloves: Array.isArray(j.orgBody.gloves) ? j.orgBody.gloves.map((x: any) => String(x || '')).filter(Boolean) : [],
                savedAt: j.orgBody.savedAt || '',
            } : undefined,
        };
    } catch {
...
async function reconcileAccountPoolInject(reason: string, opts?: { force?: boolean }): Promise<{ ok: boolean; okCount: number; total: number; skipped: boolean }> {
    ...
    const hasSystemItems = !!(ws.token || bridgeToken || getDaoRulesText());
    if (!hasSystemItems && !p.enabled) { poolReconcileLog('trigger=' + reason + ' skip=disabled+no-sys'); return skip; }
    const hasUserItems = !!(p.secrets.length || p.knowledge.length || p.playbooks.length || p.mcps.length || (p.automations && p.automations.length) || typeof p.messageLimit === 'number' || p.messageLimitAuto);
